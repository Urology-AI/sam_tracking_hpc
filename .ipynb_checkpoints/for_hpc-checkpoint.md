...
These two files are not part of the upstream SAM 2 repo:

```bash
# Adjust source path if the development repo moves
...
cp $SRC/index_brush.html      /sc/arion/projects/video_rarp/neel_projects/sam2/
```

> **Note:** The frontend (`index_brush.html`) is a placeholder and will be replaced
> with a proper React application. Only `sam2_brush_server.py` is the long-term
> entrypoint.
...
## Step 7 — Access from a local machine (SSH tunnel)
