...
# Always ensure server runtime deps are present (catches existing envs missing a package)
python -c "import multipart" 2>/dev/null || pip install --quiet python-multipart
python -c "import cv2"       2>/dev/null || pip install --quiet opencv-python-headless
python -c "import fastapi"   2>/dev/null || pip install --quiet fastapi uvicorn
